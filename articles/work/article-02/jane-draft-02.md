...
action=close means that, under the applicable session-end semantics, the allowed session closed normally. It is not a general synonym for “allow” across every log family.
...
In receiver_unavailable, the simulator sent the same 180 bytes to 127.0.0.1:25140, where no application was listening. The local sendto() call succeeded. The experiment therefore recorded a successful local attempt but no corresponding application receipt at the destination.
